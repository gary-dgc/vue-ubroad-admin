...
import { setLocCache, getLocCache } from '@/utils/cache';
  ...
  actions: {
      ...
      const list = await Promise.all(
        menuLists.map(async (item) => {
          const route = {
            ...
            component: item.component != 'Layout' ? await this.tranformToRoute(item.component) : null,
            sort: item.sort,
            path: item.path,
            name: item.name,
    ...
    async tranformToRoute(component) {
      ...
      const mateKey = routeKeys.filter((item) => {
        ...
        const endSign = component.endsWith('.vue') || component.endsWith('.jsx');
      ...
      const routerMap = list.map((item) => {
        if (item.children && item.children.length) {
          item.children = item.children.map((route) => {
            ...
            route.path = route.path.indexOf('//') != -1 ? route.path.replace(/\/\//g, '/') : route.path;
            route.path = route.path[0] == '/' ? route.path : '/' + route.path;
