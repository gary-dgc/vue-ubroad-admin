    ...
    <div class="sys-pdf-setting" ref="pdfSettingRef">
      <div class="sys-pdf-setting-left">
        <Tooltip>
          ...
          <menu-fold-outlined v-if="collapsed" class="trigger" @click="handleCollapsed" />
          <menu-unfold-outlined v-else class="trigger" @click="handleCollapsed" />
        </Tooltip>
        <div>pdf预览</div>
      </div>
    ...
    <div class="sys-pdf-canvas" ref="pdfRender">
      <div class="sys-pdf-canvas-list" ref="listRef">
        <div v-show="collapsed">
          <canvas v-for="item in totalNum" :key="item" :id="`pdf-directory__${item}`" />
        </div>
      </div>
      <div class="sys-pdf-canvas-content">
        <canvas v-for="item in totalNum" :key="item" :id="`pdf-canvas__${item}`" />
      </div>
    </div>
  </div>
</template>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const getPdfDocument = (times) => {
        pdfjs.getDocument('compressed.tracemonkey-pldi-09.pdf').promise.then((doc) => {
          pdfDoc = doc;
      ...
      const renderPage = (pageSize) => {
        pdfDoc.getPage(pageSize).then(async (page) => {
          ...
          canvas.width = Math.floor(viewport.viewBox[2] * outputScale * scaleNum.value);
          canvas.height = Math.floor(viewport.viewBox[3] * outputScale * scaleNum.value);
          canvas.style.width = Math.floor(viewport.viewBox[2] * scaleNum.value) + 'px';
          canvas.style.height = Math.floor(viewport.viewBox[3] * scaleNum.value) + 'px';
          canvas.style.position = 'absolute';
          canvas.style.left = '0';
          canvas.style.right = '0';
          canvas.style.margin = 'auto';
          canvas.style.top = `${Math.floor(viewport.viewBox[3] * scaleNum.value * (pageSize - 1)) + pageSize * 10}px`;
          canvas.style.transform = `rotateY(180deg) rotate(180deg)`;
          canvas.style.padding = '0';
          const transform =
            outputScale !== 1 ? [outputScale * scaleNum.value, 0, 0, outputScale * scaleNum.value, 0, 0] : null;
          const renderContext = {
      ...
      const renderLeftDirectory = (pageSize) => {
        pdfDoc.getPage(pageSize).then(async (page) => {
          ...
          canvas.style.right = '0';
          canvas.style.margin = 'auto';
          canvas.style.top = `${Math.floor(viewport.viewBox[3] * scale * (pageSize - 1)) + pageSize * 26}px`;
          canvas.style.transform = `rotateY(180deg) rotate(180deg)`;
          canvas.style.padding = '0';
          ...
          div.style.top = `${
            Math.floor(viewport.viewBox[3] * scale * (pageSize - 1)) + pageSize * 26 + viewport.viewBox[3] * scale + 3
          }px`;
          ...
          canvas.addEventListener('click', (event) => {
            const selectedDom =
              event.target.id && event.target.id.indexOf('__') > -1 ? event.target.id.split('__')[1] : '';
            scrollToStart(selectedDom);
          });
          const transform = outputScale !== 1 ? [outputScale * scale, 0, 0, outputScale * scale, 0, 0] : null;
          const renderContext = {
            canvasContext: context,
            transform,
            viewport: viewport,
      ...
      const handleChange = (e) => {
        ...
        if (inputValue && inputValue > 0) {
          if (inputValue.toString().indexOf('.') > -1) {
            ...
            scrollToStart(inputValue > totalNum.value ? totalNum.value : inputValue);
          }
        } else {
          inputPage.value = 1;
          scrollToStart(1);
      ...
      const onAppointPage = () => {
        ...
        scrollToStart(inputPage.value > totalNum.value ? totalNum.value : inputPage.value);
      };
